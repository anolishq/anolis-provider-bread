...
inventory::ProbeRecord probe_device(crumbs::Session &session, uint8_t address) {
    ...
    if (version_frame.opcode != 0x00) {
        probe.status = inventory::ProbeStatus::VersionReadFailed;
        probe.detail = "unexpected opcode in version reply: " + hex_byte(version_frame.opcode);
        logging::warning("probe " + format_i2c_address(static_cast<int>(address)) +
                         " unexpected version reply opcode " + hex_byte(version_frame.opcode));
        return probe;
    }
    ...
    if (!inventory::try_parse_bread_type(version_frame.type_id, device_type)) {
        ...
        logging::info("probe " + format_i2c_address(static_cast<int>(address)) + " unsupported type_id " +
                      hex_byte(version_frame.type_id));
        return probe;
    }

    if (version_frame.payload.size() < 5u) {
        probe.status = inventory::ProbeStatus::VersionReadFailed;
        probe.detail = "version payload too short (" + std::to_string(version_frame.payload.size()) + " bytes)";
        logging::warning("probe " + format_i2c_address(static_cast<int>(address)) + " version payload too short (" +
    ...
    uint8_t mod_minor = 0;
    uint8_t mod_patch = 0;
    if (bread_parse_version(version_frame.payload.data(), static_cast<uint8_t>(version_frame.payload.size()),
                            &crumbs_ver, &mod_major, &mod_minor, &mod_patch) != 0) {
        probe.status = inventory::ProbeStatus::VersionReadFailed;
        probe.detail = "failed to parse version payload";
        logging::warning("probe " + format_i2c_address(static_cast<int>(address)) + " failed to parse version payload");
        return probe;
    }
    ...
    const inventory::ProbeStatus compat =
        inventory::evaluate_version_compatibility(version_frame.type_id, version, &compat_detail);
    if (compat != inventory::ProbeStatus::Supported) {
        probe.status = compat;
        probe.detail = compat_detail;
        logging::warning("probe " + format_i2c_address(static_cast<int>(address)) + " compat failed: " + compat_detail);
        return probe;
    }
    ...
    if (!caps_status || caps_frame.opcode != BREAD_OP_GET_CAPS) {
        ...
        if (!caps_status) {
            logging::warning("probe " + format_i2c_address(static_cast<int>(address)) + " caps query failed (" +
                             caps_status.message + "), using baseline fallback");
        } else {
            logging::warning("probe " + format_i2c_address(static_cast<int>(address)) +
        ...
        if (bread_caps_parse_payload(caps_frame.payload.data(), static_cast<uint8_t>(caps_frame.payload.size()),
                                     &caps_result) == 0) {
    ...
    diag << "probe " << format_i2c_address(static_cast<int>(address)) << " type=" << hex_byte(version_frame.type_id)
         << " crumbs=" << crumbs_ver << " module=" << static_cast<int>(mod_major) << "." << static_cast<int>(mod_minor)
         << "." << static_cast<int>(mod_patch)
...
DiscoveryResult run_discovery(crumbs::Session &session, const ProviderConfig &config) {
    ...
    if (config.discovery_mode == DiscoveryMode::Scan) {
        ...
        logging::info("scan found " + std::to_string(scan_results.size()) + " CRUMBS device(s)");
    ...
    if (!build.unsupported_probes.empty()) {
        ...
        for (const auto &p : build.unsupported_probes) {
            logging::warning("  excluded " + format_i2c_address(p.address) +
                             " status=" + inventory::to_string(p.status) + " detail=" + p.detail);
        }
    }
