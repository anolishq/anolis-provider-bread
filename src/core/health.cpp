...
ProviderHealth make_provider_health(const runtime::RuntimeState &state) {
    ...
    provider.set_state(!state.ready || degraded ? ProviderHealth::STATE_DEGRADED : ProviderHealth::STATE_OK);
    if (degraded) {
        provider.set_message("provider ready but " + std::to_string(state.missing_expected_ids.size()) +
    ...
    provider.mutable_metrics()->insert({"unsupported_probe_count", std::to_string(state.unsupported_probe_count)});
    provider.mutable_metrics()->insert({"missing_expected_count", std::to_string(state.missing_expected_ids.size())});
    return provider;
}
...
void populate_wait_ready(const runtime::RuntimeState &state, WaitReadyResponse &response) {
    ...
    response.mutable_diagnostics()->insert({"unsupported_probe_count", std::to_string(state.unsupported_probe_count)});
    response.mutable_diagnostics()->insert(
        {"missing_expected_count", std::to_string(state.missing_expected_ids.size())});
