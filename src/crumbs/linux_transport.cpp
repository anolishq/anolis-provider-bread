...
SessionStatus LinuxTransport::open(const SessionOptions &options) {
    ...
    if (open_) {
        return SessionStatus::failure(SessionErrorCode::AlreadyOpen, "CRUMBS Linux transport is already open");
    }
    ...
    if (rc != 0) {
        return failure_from_errno(SessionErrorCode::OpenFailed,
                                  "failed to open Linux I2C bus '" + options.bus_path + "'", rc, saved_errno);
    }
...
SessionStatus LinuxTransport::scan(const ScanOptions &options, std::vector<ScanResult> &out) {
    if (!open_) {
    ...
    const int rc = crumbs_linux_scan_for_crumbs_with_types(&ctx_, &i2c_, options.start_address, options.end_address,
                                                           options.strict ? 1 : 0, addresses.data(), types.data(),
                                                           options.max_results, timeout_us_);
    const int saved_errno = errno;
    if (rc < 0) {
        return failure_from_errno(SessionErrorCode::ScanFailed, "failed to scan CRUMBS devices", rc, saved_errno);
    }
...
SessionStatus LinuxTransport::send(uint8_t address, const RawFrame &frame) {
    ...
    errno = 0;
    const int rc = crumbs_controller_send(&ctx_, address, &message, crumbs_linux_i2c_write, &i2c_);
    const int saved_errno = errno;
    if (rc != 0) {
        return failure_from_errno(SessionErrorCode::WriteFailed,
                                  "failed to send CRUMBS frame to " + format_i2c_address(static_cast<int>(address)), rc,
                                  saved_errno);
    }
...
SessionStatus LinuxTransport::read(uint8_t address, RawFrame &frame, uint32_t timeout_us) {
    ...
    errno = 0;
    const int bytes_read = crumbs_linux_read(&i2c_, address, buffer, sizeof(buffer), timeout_us);
    const int saved_errno = errno;
    if (bytes_read < 0) {
        const SessionErrorCode code =
            saved_errno == ETIMEDOUT ? SessionErrorCode::Timeout : SessionErrorCode::ReadFailed;
