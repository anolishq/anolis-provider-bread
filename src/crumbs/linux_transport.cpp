...
SessionStatus LinuxTransport::open(const SessionOptions &options) {
    ...
    if(rc != 0) {
        return failure_from_errno(
            SessionErrorCode::OpenFailed,
            "failed to open Linux I2C bus '" + options.bus_path + "'",
            rc,
            saved_errno);
    }
...
bool LinuxTransport::is_open() const {
    return open_;
}
...
SessionStatus LinuxTransport::scan(const ScanOptions &options, std::vector<ScanResult> &out) {
    ...
    if(rc < 0) {
        return failure_from_errno(
            SessionErrorCode::ScanFailed,
            "failed to scan CRUMBS devices",
            rc,
            saved_errno);
    }
...
SessionStatus LinuxTransport::send(uint8_t address, const RawFrame &frame) {
    ...
    errno = 0;
    const int rc = crumbs_controller_send(
        &ctx_,
        address,
        &message,
...
SessionStatus LinuxTransport::read(uint8_t address, RawFrame &frame, uint32_t timeout_us) {
    ...
    errno = 0;
    const int bytes_read = crumbs_linux_read(
        &i2c_,
        address,
        buffer,
        sizeof(buffer),
    ...
    if(rc != 0) {
        ...
        return SessionStatus::failure(
            code,
            "failed to decode CRUMBS reply from " + format_i2c_address(static_cast<int>(address)),
            rc);
    }
