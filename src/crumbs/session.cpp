...
std::string describe_failure(const std::string &operation, const SessionStatus &status,
                             const std::optional<uint8_t> &address) {
    std::ostringstream out;
    out << operation << " failed" << describe_address(address) << " code=" << to_string(status.code)
...
SessionStatus execute_with_retry(const SessionOptions &options, const std::string &operation,
                                 const std::optional<uint8_t> &address, Fn &&fn) {
    const int max_attempts = std::max(1, options.retry_count + 1);
    SessionStatus last = SessionStatus::failure(SessionErrorCode::TransportError, operation + " did not run");
...
Session::Session(Transport &transport, SessionOptions options) : transport_(transport), options_(std::move(options)) {}
...
SessionStatus Session::scan(const ScanOptions &options, std::vector<ScanResult> &out) {
    ...
    out.clear();
    return execute_with_retry(options_, "scan", std::nullopt, [&]() { return transport_.scan(options, out); });
}
...
SessionStatus Session::validate_open_options() const {
    ...
    if (options_.retry_count < 0) {
        return SessionStatus::failure(SessionErrorCode::InvalidArgument, "session retry_count must be non-negative");
    }
...
SessionStatus Session::validate_scan_options(const ScanOptions &options) const {
    ...
    if (options.start_address > options.end_address) {
        return SessionStatus::failure(SessionErrorCode::InvalidArgument, "scan start_address must be <= end_address");
    }
    if (options.max_results == 0u) {
        return SessionStatus::failure(SessionErrorCode::InvalidArgument, "scan max_results must be greater than zero");
    }
    return SessionStatus::success();
}
...
SessionStatus Session::validate_address(uint8_t address) const {
    if (address < kMinI2cAddress || address > kMaxI2cAddress) {
        return SessionStatus::failure(SessionErrorCode::InvalidArgument, "I2C address is outside the supported range");
    }
    return SessionStatus::success();
}
