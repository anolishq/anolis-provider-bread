...
inline void append_u8(std::vector<uint8_t> &buf, uint8_t v) { buf.push_back(v); }
...
inline void append_i16_le(std::vector<uint8_t> &buf, int16_t v) { append_u16_le(buf, static_cast<uint16_t>(v)); }
 ...
 * `CODE_UNAVAILABLE`.
 */
inline AdapterReadResult read_error_from_session(const crumbs::SessionStatus &status, const std::string &op) {
 ...
 * handlers.
 */
inline AdapterCallResult call_error_from_session(const crumbs::SessionStatus &status, const std::string &op) {
    using S = anolis::deviceprovider::v1::Status;
    if (status.code == crumbs::SessionErrorCode::Timeout) {
        return {false, S::CODE_DEADLINE_EXCEEDED, op + " timed out: " + status.message};
    }
