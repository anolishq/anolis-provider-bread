...
std::string format_module_version(const ModuleVersion &version) {
    std::ostringstream out;
    out << static_cast<int>(version.module_major) << '.' << static_cast<int>(version.module_minor) << '.'
...
std::string format_crumbs_version(uint16_t value) {
    std::ostringstream out;
    out << (value / 10000u) << '.' << ((value / 100u) % 100u) << '.' << (value % 100u);
    return out.str();
}

ArgSpec *add_arg(FunctionSpec &function, const std::string &name, anolis::deviceprovider::v1::ValueType type,
                 const std::string &description, bool required, const std::string &unit = "") {
...
FunctionSpec *add_function(CapabilitySet &caps, uint32_t function_id, const std::string &name,
...
void add_signal(CapabilitySet &caps, const std::string &signal_id, const std::string &description,
                anolis::deviceprovider::v1::ValueType type, const std::string &unit = "") {
...
CapabilitySet build_rlht_capabilities(uint32_t flags) {
    ...
    if ((flags & RLHT_CAP_SETPOINT_CONTROL) != 0u) {
        auto *set_setpoints =
            add_function(caps, 2, "set_setpoints", "Set RLHT channel temperature setpoints.", CAT_CONFIG);
        auto *sp1_arg = add_arg(*set_setpoints, "setpoint1_c", VT_DOUBLE, "Channel 1 target temperature.", true, "C");
        sp1_arg->set_min_double(-3276.8);
        sp1_arg->set_max_double(3276.7);
        auto *sp2_arg = add_arg(*set_setpoints, "setpoint2_c", VT_DOUBLE, "Channel 2 target temperature.", true, "C");
    ...
    if ((flags & RLHT_CAP_PID_TUNING) != 0u) {
        ...
        auto *kp1_arg = add_arg(*set_pid, "kp1_x10", VT_UINT64, "Channel 1 proportional gain x10.", true);
        kp1_arg->set_min_uint64(0);
        kp1_arg->set_max_uint64(255);
        auto *ki1_arg = add_arg(*set_pid, "ki1_x10", VT_UINT64, "Channel 1 integral gain x10.", true);
        ki1_arg->set_min_uint64(0);
        ki1_arg->set_max_uint64(255);
        auto *kd1_arg = add_arg(*set_pid, "kd1_x10", VT_UINT64, "Channel 1 derivative gain x10.", true);
        kd1_arg->set_min_uint64(0);
        kd1_arg->set_max_uint64(255);
        auto *kp2_arg = add_arg(*set_pid, "kp2_x10", VT_UINT64, "Channel 2 proportional gain x10.", true);
        kp2_arg->set_min_uint64(0);
        kp2_arg->set_max_uint64(255);
        auto *ki2_arg = add_arg(*set_pid, "ki2_x10", VT_UINT64, "Channel 2 integral gain x10.", true);
        ki2_arg->set_min_uint64(0);
        ki2_arg->set_max_uint64(255);
        auto *kd2_arg = add_arg(*set_pid, "kd2_x10", VT_UINT64, "Channel 2 derivative gain x10.", true);
    ...
    if ((flags & RLHT_CAP_PERIOD_CONTROL) != 0u) {
        ...
        auto *p1_arg = add_arg(*set_periods, "period1_ms", VT_UINT64, "Relay 1 period.", true, "ms");
        p1_arg->set_min_uint64(0);
        p1_arg->set_max_uint64(65535);
        auto *p2_arg = add_arg(*set_periods, "period2_ms", VT_UINT64, "Relay 2 period.", true, "ms");
    ...
    if ((flags & RLHT_CAP_TC_SELECT) != 0u) {
        ...
        auto *tc1_arg = add_arg(*set_tc_select, "tc1_index", VT_UINT64, "Channel 1 thermocouple index.", true);
        tc1_arg->set_min_uint64(0);
        tc1_arg->set_max_uint64(255);
        auto *tc2_arg = add_arg(*set_tc_select, "tc2_index", VT_UINT64, "Channel 2 thermocouple index.", true);
    ...
    if ((flags & RLHT_CAP_OPEN_DUTY_CONTROL) != 0u) {
        auto *set_open_duty =
            add_function(caps, 6, "set_open_duty_pct", "Set RLHT open-loop duty percentages.", CAT_ACTUATE);
        auto *d1_arg = add_arg(*set_open_duty, "duty1_pct", VT_UINT64, "Channel 1 duty cycle percentage.", true, "%");
        d1_arg->set_min_uint64(0);
        d1_arg->set_max_uint64(100);
        auto *d2_arg = add_arg(*set_open_duty, "duty2_pct", VT_UINT64, "Channel 2 duty cycle percentage.", true, "%");
...
CapabilitySet build_dcmt_capabilities(uint32_t flags) {
    ...
    if ((flags & DCMT_CAP_OPEN_LOOP_CONTROL) != 0u) {
        ...
        auto *m1_arg = add_arg(*set_open_loop, "motor1_pwm", VT_INT64, "Motor 1 PWM command.", true);
        m1_arg->set_min_int64(-255);
        m1_arg->set_max_int64(255);
        auto *m2_arg = add_arg(*set_open_loop, "motor2_pwm", VT_INT64, "Motor 2 PWM command.", true);
    ...
    if ((flags & (DCMT_CAP_CLOSED_LOOP_POSITION | DCMT_CAP_CLOSED_LOOP_SPEED)) != 0u) {
        ...
        auto *t1_arg = add_arg(*set_setpoint, "motor1_target", VT_INT64, "Motor 1 target value.", true);
        t1_arg->set_min_int64(INT16_MIN);
        t1_arg->set_max_int64(INT16_MAX);
        auto *t2_arg = add_arg(*set_setpoint, "motor2_target", VT_INT64, "Motor 2 target value.", true);
    ...
    if ((flags & DCMT_CAP_PID_TUNING) != 0u) {
        ...
        auto *kp1_arg = add_arg(*set_pid, "kp1_x10", VT_UINT64, "Motor 1 proportional gain x10.", true);
        kp1_arg->set_min_uint64(0);
        kp1_arg->set_max_uint64(255);
        auto *ki1_arg = add_arg(*set_pid, "ki1_x10", VT_UINT64, "Motor 1 integral gain x10.", true);
        ki1_arg->set_min_uint64(0);
        ki1_arg->set_max_uint64(255);
        auto *kd1_arg = add_arg(*set_pid, "kd1_x10", VT_UINT64, "Motor 1 derivative gain x10.", true);
        kd1_arg->set_min_uint64(0);
        kd1_arg->set_max_uint64(255);
        auto *kp2_arg = add_arg(*set_pid, "kp2_x10", VT_UINT64, "Motor 2 proportional gain x10.", true);
        kp2_arg->set_min_uint64(0);
        kp2_arg->set_max_uint64(255);
        auto *ki2_arg = add_arg(*set_pid, "ki2_x10", VT_UINT64, "Motor 2 integral gain x10.", true);
        ki2_arg->set_min_uint64(0);
        ki2_arg->set_max_uint64(255);
...
InventoryDevice build_device(const ProviderConfig &config, const ProbeRecord &probe, DeviceType type,
                             InventorySource source, bool expected, const std::string &device_id,
    ...
    for (ProbeRecord probe : sorted_probes) {
        ...
        if (match_index.has_value()) {
            ...
            if (spec.type != type) {
                ...
                probe.detail = "configured type '" + to_string(spec.type) + "' does not match probed type '" +
                               to_string(type) + "'";
                result.unsupported_probes.push_back(std::move(probe));
                continue;
            }
...
std::vector<InventoryDevice> build_seed_inventory(const ProviderConfig &config) {
    ...
    return build_inventory_from_probes(config, build_seed_probes(config), InventorySource::ConfigSeeded)
        .supported_devices;
}
