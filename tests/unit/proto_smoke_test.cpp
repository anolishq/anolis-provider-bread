...
TEST(ProtoSmokeTest, InMemoryProviderLoopCanRoundTripHello) {
    ...
    ASSERT_TRUE(anolis_provider_bread::transport::write_frame(
        outbound, reinterpret_cast<const uint8_t *>(request_payload.data()), request_payload.size(), io_error))
        << io_error;
    ...
    ASSERT_TRUE(anolis_provider_bread::transport::write_frame(
        framed_response, reinterpret_cast<const uint8_t *>(response_payload.data()), response_payload.size(), io_error))
        << io_error;
