...
public:
    ...
    SessionStatus read(uint8_t address, RawFrame &frame, uint32_t timeout_us) override {
        ...
        if (read_actions.empty()) {
            return SessionStatus::failure(SessionErrorCode::ReadFailed, "no fake read action configured");
        }
...
TEST(CrumbsSessionTest, QueryReadStopsOnNonRetryableFailure) {
    ...
    transport.send_actions.push_back(SessionStatus::failure(SessionErrorCode::InvalidArgument, "bad frame"));
