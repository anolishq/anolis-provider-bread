...
public:
    ...
    crumbs::SessionStatus scan(const crumbs::ScanOptions &, std::vector<crumbs::ScanResult> &) override {
        return crumbs::SessionStatus::success();
    }
    ...
    crumbs::SessionStatus read(uint8_t, crumbs::RawFrame &out, uint32_t) override {
        ...
        if (it == read_replies.end()) {
            return crumbs::SessionStatus::failure(crumbs::SessionErrorCode::ReadFailed, "no scripted reply for opcode");
        }
        out = it->second;
        return crumbs::SessionStatus::success();
    }
...
static std::vector<uint8_t> make_state_payload(uint8_t mode, int16_t m1_pwm, int16_t m2_pwm, int16_t sp1, int16_t sp2,
                                               int16_t pos1, int16_t pos2, int16_t spd1, int16_t spd2, uint8_t brakes,
                                               uint8_t estop) {
...
static std::vector<uint8_t> make_open_loop_payload(int16_t m1_pwm, int16_t m2_pwm, uint8_t brakes, uint8_t estop) {
    return make_state_payload(DCMT_MODE_OPEN_LOOP, m1_pwm, m2_pwm, BREAD_INVALID_I16, BREAD_INVALID_I16, 0, 0,
                              BREAD_INVALID_I16, BREAD_INVALID_I16, brakes, estop);
}

static std::vector<uint8_t> make_closed_position_payload(int16_t sp1, int16_t sp2, int16_t pos1, int16_t pos2,
                                                         uint8_t brakes, uint8_t estop) {
    return make_state_payload(DCMT_MODE_CLOSED_POSITION, 0, 0, sp1, sp2, pos1, pos2, BREAD_INVALID_I16,
                              BREAD_INVALID_I16, brakes, estop);
}

static std::vector<uint8_t> make_closed_speed_payload(int16_t sp1, int16_t sp2, int16_t spd1, int16_t spd2,
                                                      uint8_t brakes, uint8_t estop) {
    return make_state_payload(DCMT_MODE_CLOSED_SPEED, 0, 0, sp1, sp2, 0, 0, spd1, spd2, brakes, estop);
}
...
struct DcmtAdapterTest : public ::testing::Test {
    ...
    static ValueMap make_args(std::initializer_list<std::pair<std::string, anolis::deviceprovider::v1::Value>> pairs) {
        ValueMap m;
...
TEST_F(DcmtAdapterTest, ReadSignals_OpenLoop_ReturnsAllSignals) {
    ...
    const auto result = read_signals(session, device,
                                     {"mode", "motor1_target", "motor2_target", "motor1_value", "motor2_value",
                                      "motor1_brake", "motor2_brake", "estop"});
...
TEST_F(DcmtAdapterTest, ReadSignals_ClosedPosition_EncoderFeedbackIsIndependent) {
    ...
    script_state_reply(make_closed_position_payload(1000, -1000, 980, -990, 0x00, 0x00));

    const auto result =
        read_signals(session, device, {"mode", "motor1_target", "motor1_value", "motor2_target", "motor2_value"});
...
TEST_F(DcmtAdapterTest, ReadSignals_ClosedSpeed_ModeStringCorrect) {
    script_state_reply(make_closed_speed_payload(500, 500, 498, 501, 0x00, 0x00));
...
TEST_F(DcmtAdapterTest, ReadSignals_Estop_DecodedCorrectly) {
    ...
    // explicitly.
    const auto result = read_signals(session, device, {"estop", "motor1_brake", "motor2_brake"});
    ASSERT_TRUE(result.ok);
...
TEST_F(DcmtAdapterTest, ReadSignals_InvalidClosedSpeedSentinel_ReturnsInternal) {
    script_state_reply(make_state_payload(DCMT_MODE_CLOSED_SPEED, 0, 0, 100, 100, 0, 0, BREAD_INVALID_I16,
...
TEST_F(DcmtAdapterTest, ReadSignals_WrongTypeId_ReturnsInternal) {
    transport.read_replies[DCMT_OP_GET_STATE] =
        crumbs::RawFrame{0x99u, DCMT_OP_GET_STATE, make_open_loop_payload(0, 0, 0, 0)};
...
TEST_F(DcmtAdapterTest, ReadSignals_SessionFails_ReturnsUnavailable) {
    ...
    EXPECT_FALSE(result.ok);
    EXPECT_EQ(result.error_code, anolis::deviceprovider::v1::Status::CODE_UNAVAILABLE);
}
...
TEST_F(DcmtAdapterTest, Call_SetOpenLoop_PwmOverRange_ReturnsOutOfRange) {
    ...
    EXPECT_FALSE(result.ok);
    EXPECT_EQ(result.error_code, anolis::deviceprovider::v1::Status::CODE_OUT_OF_RANGE);
...
TEST_F(DcmtAdapterTest, Call_SetMode_OpenLoop_SendsCorrectByte) {
    ...
    EXPECT_EQ(transport.sent_frames.at(0).second.payload.at(0), static_cast<uint8_t>(DCMT_MODE_OPEN_LOOP));
}
...
TEST_F(DcmtAdapterTest, Call_SetMode_ClosedPosition_SendsCorrectByte) {
    ...
    EXPECT_EQ(transport.sent_frames.at(0).second.payload.at(0), static_cast<uint8_t>(DCMT_MODE_CLOSED_POSITION));
}
...
TEST_F(DcmtAdapterTest, Call_SetMode_InvalidMode_ReturnsInvalidArgument) {
    ...
    EXPECT_FALSE(result.ok);
    EXPECT_EQ(result.error_code, anolis::deviceprovider::v1::Status::CODE_INVALID_ARGUMENT);
}
...
TEST_F(DcmtAdapterTest, Call_UnknownFunctionId_ReturnsNotFound) {
    ...
    EXPECT_FALSE(result.ok);
    EXPECT_EQ(result.error_code, anolis::deviceprovider::v1::Status::CODE_NOT_FOUND);
}
...
TEST_F(DcmtAdapterTest, Call_SendTimesOut_ReturnsDeadlineExceeded) {
    ...
    const auto args = make_args({{"motor1_pwm", make_int64_val(100)}, {"motor2_pwm", make_int64_val(-50)}});
    const auto result = call(session, device, 1u, args);
