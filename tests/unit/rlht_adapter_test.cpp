...
struct RlhtAdapterTest : public ::testing::Test {
    ...
    static std::vector<uint8_t> make_state_payload(uint8_t mode, uint8_t flags, int16_t t1_dc, int16_t t2_dc,
                                                   int16_t sp1_dc, int16_t sp2_dc, uint16_t on1_ms, uint16_t on2_ms,
                                                   uint16_t period1_ms, uint16_t period2_ms, uint8_t tc_select) {
...
TEST_F(RlhtAdapterTest, ReadSignals_ClosedLoopState_ReturnsAllSignals) {
    ...
    script_state_reply(make_state_payload(RLHT_MODE_CLOSED_LOOP, RLHT_FLAG_RELAY1_ON, 2505,
    ...
    const auto result = read_signals(session, device,
                                     {"mode", "t1_c", "t2_c", "setpoint1_c", "setpoint2_c", "period1_ms", "period2_ms",
                                      "relay1_on", "relay2_on", "estop"});
...
TEST_F(RlhtAdapterTest, ReadSignals_OpenLoopState_ReturnsOpenLoopMode) {
    script_state_reply(make_state_payload(RLHT_MODE_OPEN_LOOP, RLHT_FLAG_ESTOP, 0, 0, 0, 0, 0, 0, 1000, 1000, 0x00));
...
TEST_F(RlhtAdapterTest, ReadSignals_DefaultSet_IsCuratedSubset) {
    ...
    script_state_reply(make_state_payload(RLHT_MODE_CLOSED_LOOP, RLHT_FLAG_RELAY1_ON, 2505, 2510, 2000, 2010, 500, 600,
                                          5000, 6000, 0x00));
...
TEST_F(RlhtAdapterTest, ReadSignals_SubsetRequest_ReturnsOnlyRequestedSignals) {
    script_state_reply(make_state_payload(RLHT_MODE_CLOSED_LOOP, 0x00, 1000, 1010, 950, 960, 0, 0, 2000, 2000, 0x00));
...
TEST_F(RlhtAdapterTest, ReadSignals_NegativeTemperature_ParsedCorrectly) {
    // -45.3°C = -453 deci-C
    script_state_reply(
        make_state_payload(RLHT_MODE_CLOSED_LOOP, 0x00, static_cast<int16_t>(-453), 0, 0, 0, 0, 0, 1000, 1000, 0x00));
...
TEST_F(RlhtAdapterTest, Call_SetMode_OpenLoop_SendsCorrectByte) {
    ...
    ASSERT_TRUE(result.ok);
    EXPECT_EQ(transport.sent_frames.at(0).second.payload.at(0), static_cast<uint8_t>(RLHT_MODE_OPEN_LOOP));
}
...
TEST_F(RlhtAdapterTest, Call_SetSetpoints_SendsCorrectPayload) {
    ...
    EXPECT_EQ(transport.sent_frames.at(0).second.opcode, static_cast<uint8_t>(RLHT_OP_SET_SETPOINTS));
...
TEST_F(RlhtAdapterTest, Call_SetSetpoints_OutOfRange_ReturnsOutOfRange) {
    ...
    EXPECT_EQ(result.error_message, "setpoint values must be in [-3276.8, 3276.7] C");
}
