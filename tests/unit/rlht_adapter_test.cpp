...
struct RlhtAdapterTest : public ::testing::Test {
    ...
    static std::vector<uint8_t> make_state_payload(
        uint8_t mode, uint8_t flags,
        int16_t t1_dc, int16_t t2_dc,
        int16_t sp1_dc, int16_t sp2_dc,
        uint16_t on1_ms, uint16_t on2_ms,
        uint16_t period1_ms, uint16_t period2_ms,
        uint8_t tc_select) {
...
TEST_F(RlhtAdapterTest, ReadSignals_ClosedLoopState_ReturnsAllSignals) {
    ...
    script_state_reply(make_state_payload(
        RLHT_MODE_CLOSED_LOOP, RLHT_FLAG_RELAY1_ON,
        2505, 2510,   // t1=250.5°C, t2=251.0°C (deci-C)
...
TEST_F(RlhtAdapterTest, ReadSignals_OpenLoopState_ReturnsOpenLoopMode) {
    script_state_reply(make_state_payload(
        RLHT_MODE_OPEN_LOOP, RLHT_FLAG_ESTOP,
        0, 0, 0, 0, 0, 0, 1000, 1000, 0x00));
...
TEST_F(RlhtAdapterTest, ReadSignals_SubsetRequest_ReturnsOnlyRequestedSignals) {
    script_state_reply(make_state_payload(
        ...
        1000, 1010, 950, 960, 0, 0, 2000, 2000, 0x00));
...
TEST_F(RlhtAdapterTest, ReadSignals_NegativeTemperature_ParsedCorrectly) {
    ...
    script_state_reply(make_state_payload(
        RLHT_MODE_CLOSED_LOOP, 0x00,
        static_cast<int16_t>(-453), 0, 0, 0, 0, 0, 1000, 1000, 0x00));
