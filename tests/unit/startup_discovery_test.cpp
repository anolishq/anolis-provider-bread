    ...
    crumbs::SessionStatus read(uint8_t address, crumbs::RawFrame &frame, uint32_t /*timeout_us*/) override {
        ...
        if (err_it != read_errors.end()) {
            ...
            if (opc_it != err_it->second.end()) {
                return crumbs::SessionStatus::failure(opc_it->second, "scripted read error");
            }
        }
        ...
        if (addr_it == replies.end()) {
            return crumbs::SessionStatus::failure(crumbs::SessionErrorCode::ReadFailed, "no script for address");
        }
        auto opc_it = addr_it->second.find(pending_opcode_);
        if (opc_it == addr_it->second.end()) {
            return crumbs::SessionStatus::failure(crumbs::SessionErrorCode::ReadFailed, "no script for opcode");
        }
...
std::vector<uint8_t> make_version_payload(uint16_t crumbs_ver, uint8_t major, uint8_t minor, uint8_t patch) {
    return {static_cast<uint8_t>(crumbs_ver & 0xFF), static_cast<uint8_t>((crumbs_ver >> 8) & 0xFF), major, minor,
            patch};
}
...
ProviderConfig make_manual_config(std::vector<int> addresses, std::vector<DeviceSpec> devices = {}) {
    ProviderConfig cfg;
    cfg.bus_path = "/dev/i2c-test";
...
TEST(StartupDiscoveryTest, ScanModeBuildInventoryFromSupportedRlhtDevice) {
    ...
    EXPECT_EQ(result.devices[0].capability_profile.source, inventory::CapabilitySource::Queried);
...
TEST(StartupDiscoveryTest, ManualModeUsesBaselineFallbackOnCapsFailure) {
    ...
    transport.add_reply(
        0x0A, 0x00, RLHT_TYPE_ID,
        make_version_payload(CRUMBS_VERSION, RLHT_MODULE_VER_MAJOR, RLHT_MODULE_VER_MINOR, RLHT_MODULE_VER_PATCH));
    transport.add_read_error(0x0A, BREAD_OP_GET_CAPS, crumbs::SessionErrorCode::ReadFailed);
    ...
    EXPECT_EQ(result.devices[0].capability_profile.source, inventory::CapabilitySource::BaselineFallback);
    EXPECT_TRUE(result.missing_expected_ids.empty());
...
TEST(StartupDiscoveryTest, IncompatibleCrumbsVersionIsUnsupported) {
    ...
    transport.add_reply(0x0A, 0x00, RLHT_TYPE_ID, make_version_payload(1000u, 1, 0, 0));
    ...
    EXPECT_EQ(result.unsupported_probes[0].status, inventory::ProbeStatus::IncompatibleCrumbsVersion);
...
TEST(StartupDiscoveryTest, ScanModeDcmtDeviceWithQueriedCaps) {
    ...
    transport.add_reply(
        ...
        make_version_payload(CRUMBS_VERSION, DCMT_MODULE_VER_MAJOR, DCMT_MODULE_VER_MINOR, DCMT_MODULE_VER_PATCH));

    const uint32_t level3_flags = DCMT_CAP_OPEN_LOOP_CONTROL | DCMT_CAP_BRAKE_CONTROL | DCMT_CAP_CLOSED_LOOP_POSITION |
